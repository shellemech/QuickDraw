...
function get_draw(draw_data)
{
	...
	if (/^clan/.test(thisshape))
	{
		...
		mechImg.onload = function () {
			context.drawImage(mechImg, draw_data["x"]-5, draw_data["y"]-20, 44, 56);
		}
...
// Updates - room is given map to draw to canvasback, drop down updated
...
console.log("map recieved "+ map);
        ...
        if (mapImg.src !="") {
                ...
                mapImg.onload = function() {
					console.log("Loading once img cached");
                        contextback.drawImage(mapImg, 0, 0, 950, 950);
                }
				// CLR fix
				if (mapImg.complete) contextback.drawImage(mapImg, 0, 0, 950, 950);
